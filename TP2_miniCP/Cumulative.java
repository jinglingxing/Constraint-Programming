        ...
        // TODO 2: check that the profile is not exceeding the capa otherwise throw an INCONSISTENCY
         ...
         for (int i = 0; i < start.length; i++) {
            if (!start[i].isBound()) {
                ...
                // startMax and endMin are used to check if t is in the mandatory part of the task
                int startMax = start[i].getMax(); 
                int endMin = start[i].getMin() + duration[i];
                for(int t = start[i].getMin(); t <= startMax; t++) {
                		boolean conflict = false;
                		int d=0;
                		while(conflict==false && d<=duration[i]) {
                			// We check if t is in the mandatory part
                			if(startMax<= t+d && t+d<=endMin) {
                			// t is mandatory part
                			}else {
                				int mandatoryCapacity = profile.get(profile.rectangleIndex(t+d)).height;
